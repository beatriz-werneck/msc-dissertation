# %% [markdown]
# -------------------------------------------------------------------------
## bcw_dis_16b_general_index_map_transit — figure 16 + metro / rail overlay
# -------------------------------------------------------------------------
"""
Variant of figure 16 (general X-Minute City Accessibility Index at 15/20/30 min)
with the metro and CPTM rail lines drawn on top, to show how the index relates to
the rail network. Lines are drawn with a thin white casing so they stay legible
over any viridis colour. Line geometries come from the cached OSM metro/CPTM
extracts (mapping/validation layer).
"""
# %%
import os
...
URBAN_GREY = "0.80"
METRO_C, RAIL_C = "#e41a1c", "#000000"          # metro = red, rail = black
...
# metro / CPTM rail lines (OSM), clipped to the municipality
metro = pd.read_pickle(cfg.CACHE_DIR / "osm_metro" / "metro_lines.pkl").to_crs(sp.crs)
cptm = pd.read_pickle(cfg.CACHE_DIR / "osm_cptm" / "cptm_lines.pkl").to_crs(sp.crs)
metro = gpd.clip(metro, sp)
cptm = gpd.clip(cptm, sp)
...
def draw_lines(ax, lines, color):
    if len(lines):
        lines.plot(ax=ax, color="white", linewidth=1.3, zorder=3)     # slim casing
        lines.plot(ax=ax, color=color, linewidth=0.6, zorder=4)


fig, axes = plt.subplots(1, 3, figsize=(8.27, 5.83), layout="constrained")
# reserve the bottom strip for the legend/colorbar so the maps sit higher
...
for ax, (t, col) in zip(axes, INDEX_COLS.items()):
    ...
    draw_lines(ax, cptm, RAIL_C)
    draw_lines(ax, metro, METRO_C)
...
legend_items = [Patch(facecolor=URBAN_GREY, edgecolor="none", label="Urban area"),
                ...
                Line2D([0], [0], color=RAIL_C, lw=1.6, label="Rail")]
fig.legend(handles=legend_items, loc="lower left", bbox_to_anchor=(0.02, 0.01),
           frameon=False, fontsize=9)

cfg.FIG_DIR.mkdir(parents=True, exist_ok=True)
out = cfg.FIG_DIR / f"16b_general_index_transit_res{H3_RES}_A5{SUFFIX}.png"
fig.savefig(out, dpi=300, transparent=True)
print(f"Saved figure: {out}  (metro segs {len(metro)}, cptm segs {len(cptm)})")
